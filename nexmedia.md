...
> Generated: 2026-08-26 · Full-stack analysis of `C:\Users\solo\Desktop\nexcole\nexclone`
...
| **Infra** | docker-compose: Traefik (TLS/Let's Encrypt) + Postgres 15 (:5433) + Redis 7 + MinIO (:9001/:9002) + backend (:8080) + frontend (:3000). Prod domain: nexmediaai.com |
...
| `AffiliateController` | `api/affiliate` | ApiPolicy rate-limited; profile (404 not onboarded), onboard, balances, stats, referrals paged (emails masked), commissions paged, payouts request (DTO Amount/Currency/PayoutMethod/PayoutAccount/Message; 400 invalid amount etc.), payouts list w/ signed receipts |
...
| `UsersController` | `/Users` | Index search/paginate; Details (devices/affiliate/payments/lockout); Create (auto default plan + receipt email); **AssignPlan** (extend-or-create sub, credit distribution, Payment record, affiliate commission, invoice PDF upload, receipt email); AdjustCredits; ExtendSubscription; ChangePassword; UnlockUser; Edit/Delete/BulkDelete; AddCreditAmount; UpdateSubscriptionDates; UpdateAffiliateTrackingDates |
...
| `PlansAdminController` | `/PlansAdmin` | Plan CRUD (USD/EGP, tax vs fixed-fee mutual exclusion, single-default enforcement); soft delete; ManageGateways/AddGateway (clears other defaults per currency)/RemoveGateway/ToggleGateway; **SeedTestPlans is [AllowAnonymous] GET — publicly seeds 4 plans** |
...
| Entity | Design |
|---|---|
| `AffiliateProfile` | AffiliateDisplayId "AF-{id:D5}", unique ReferralCode, TotalClicks, onboarding contacts, PolicyAcceptedAt |
...
### Hangfire recurring
`affiliate-commission-hold` — Cron.Daily midnight UTC → `AffiliateService.ProcessPendingCommissionsAsync()` (PENDING→AVAILABLE after `Affiliate.HoldPeriodDays`, default 14).
...
- affiliate/ (8): Overview, StatsGrid, ReferralLink, ReferralsTable, CommissionsTable, WithdrawalForm, PayoutsTable, OnboardingForm
...
3. `GET PlansAdmin/SeedTestPlans` is `[AllowAnonymous]` — publicly seeds 4 paid plans
...
## 11. Quick Reference — Ports & URLs

| Thing | Value |
|---|---|
| Backend dev | http://localhost:5000 (env) / :8080 (container) |
| Frontend dev | http://localhost:3000 |
| Postgres | localhost:5433 (dev container), db nexclone_dev |
| Redis | localhost:6379 (password protected) |
| MinIO | localhost:9001 API / :9002 console |
