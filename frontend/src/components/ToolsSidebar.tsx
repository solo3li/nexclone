...
export default function ToolsSidebar() {
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (consumptionRef.current && !consumptionRef.current.contains(e.target as Node)) {
        setConsumptionExpanded(false);
      }
    };
      ...
      >
        ...
        {user && (
          <div className="px-4 py-3 border-b border-white/5" dir={isRtl ? 'rtl' : 'ltr'}>
                ...
                <div className="flex items-center gap-1.5">
                  <div className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-fuchsia-500/10 border border-fuchsia-500/20 group-hover:bg-fuchsia-500/20 transition-colors">
                    ...
                    <span className="text-fuchsia-300 text-[11px] font-bold">
                      {user.wallets ? user.wallets.reduce((acc: number, w: any) => acc + w.balance, 0) : (user.availableCredits || 0)}
                    </span>
                  </div>
            ...
            {/* Consumption Mode */}
            {user.activeSubscriptions && user.activeSubscriptions.length > 1 && (
              <div className="relative mt-2" ref={consumptionRef}>
                <div 
                  className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/10 cursor-pointer hover:bg-white/10 hover:border-white/20 transition-all group"
                  onClick={() => setConsumptionExpanded(!consumptionExpanded)}
                >
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-500/20 to-cyan-500/20 flex items-center justify-center group-hover:scale-105 transition-transform">
                      <Settings2 className="w-4 h-4 text-blue-400" />
                    </div>
                    <span className="text-sm font-semibold text-white/90 group-hover:text-white transition-colors">
                      {isRtl ? 'وضع الاستهلاك' : 'Consumption Mode'}
                    </span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className="text-[11px] text-white/50">{preferredSubId ? (user.activeSubscriptions.find((s:any) => s.id === preferredSubId)?.nameAr || 'مخصص') : (isRtl ? 'تلقائي' : 'Auto')}</span>
                    <ChevronDown className={`w-3.5 h-3.5 text-white/40 group-hover:text-white/70 transition-all duration-300 ${consumptionExpanded ? 'rotate-180' : ''}`} />
                  </div>
                </div>
                ...
                <AnimatePresence>
                  {consumptionExpanded && (
                    ...
                    >
                      <div className="flex flex-col gap-1.5 p-2 max-h-80 overflow-y-auto">
                        <button onClick={() => { setPreferredSubId(null); sessionStorage.removeItem('preferredSubscriptionId'); setConsumptionExpanded(false); }} className={`w-full text-start px-3 py-2 rounded-lg text-sm transition-all ${!preferredSubId ? 'bg-blue-500/20 text-blue-300' : 'text-white/70 hover:bg-white/10'}`}>
                          {isRtl ? 'تلقائي (الأقدم أولاً)' : 'Auto (Oldest First)'}
                        </button>
                        {user.activeSubscriptions.map((sub: any) => (
                          <button 
                            key={sub.id} 
                            onClick={() => { setPreferredSubId(sub.id); sessionStorage.setItem('preferredSubscriptionId', sub.id.toString()); setConsumptionExpanded(false); }} 
                            className={`w-full text-start px-3 py-2 rounded-lg text-sm transition-all flex justify-between items-center ${preferredSubId === sub.id ? 'bg-blue-500/20 text-blue-300' : 'text-white/70 hover:bg-white/10'}`}
                          >
                            <span>{isRtl ? sub.nameAr : sub.name}</span>
                            <span className="text-[10px] text-white/40">{new Date(sub.endDate).toLocaleDateString()}</span>
                          </button>
                        ))}
                      </div>
                    </motion.div>
