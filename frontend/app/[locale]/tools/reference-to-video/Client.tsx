        ...
        <div className="order-1 lg:order-2 lg:col-span-4 space-y-5">
          <div className="sticky top-20 bg-[#0b0416]/95 border border-white/10 rounded-2xl p-5 shadow-xl space-y-4 backdrop-blur-md">
            ...
            <div className="space-y-1.5 relative" ref={modelRef}>
              ...
              {isModelDropdownOpen && (
                <div className="hidden lg:block absolute z-50 top-full mt-1.5 w-full bg-[#0d041c] border border-emerald-500/30 rounded-xl shadow-2xl overflow-hidden backdrop-blur-2xl p-1.5 space-y-1 animate-in fade-in slide-in-from-top-2 duration-150">
                  {modelOptions.map((m) => {
              ...
              <BottomSheetSelect isOpen={isModelDropdownOpen} onClose={() => setIsModelDropdownOpen(false)} title={isRtl ? "محرك الذكاء الاصطناعي" : "AI Video Model"}>
                {modelOptions.map((m) => {
                  const isSelected = selectedModelId === m.id;
                  return (
                    <button key={m.id} type="button" onClick={() => handleModelSelectWithClear(m.id)}
            ...
            <div className="space-y-1.5 relative" ref={resRef}>
              ...
              {isResDropdownOpen && (
                <div className="hidden lg:block absolute z-40 top-full mt-1.5 w-full bg-[#0d041c] border border-teal-500/30 rounded-xl shadow-2xl overflow-hidden backdrop-blur-2xl p-1.5 space-y-1 animate-in fade-in slide-in-from-top-2 duration-150">
                    ...
                    return (
                      <button key={r.id} type="button" onClick={() => { setResolution(r.id); setIsResDropdownOpen(false); }}
                        className={`w-full text-start p-2 rounded-lg transition-all flex items-center justify-between gap-2 ${
                          isSelected ? "bg-teal-600/25 text-white border border-teal-500/40" : "hover:bg-white/5 text-white/70 hover:text-white"
                        }`}>
                        <div>
