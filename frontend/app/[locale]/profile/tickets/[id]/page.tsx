...
export default function TicketChat({ params }: { params: { id: string } }) {
    ...
    if (ticket && !connection) {
      ...
      const newConnection = new signalR.HubConnectionBuilder()
        .withUrl(`${apiUrl}/hubs/ticket`, {
          // If using jwt tokens in url query for signalR auth or cookie
        })
  ...
  useEffect(() => {
    if (connection) {
      connection.start()
        .then(() => {
          connection.invoke("JoinTicketGroup", params.id);
      ...
      return () => {
        connection.invoke("LeaveTicketGroup", params.id).then(() => connection.stop());
      };
    }
  ...
  const fetchTicket = async () => {
    try {
      const res = await api.get(`/api/tickets/${params.id}`);
      setTicket(res.data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.post(`/api/tickets/${params.id}/message`, formData, {
        headers: {
  ...
  if (!ticket) {
    return (
      <div className="bg-white/5 border border-white/10 p-8 rounded-3xl text-center py-20">
        ...
        <Link href="/profile/tickets" className="text-violet-400 hover:text-violet-300">← Back to tickets</Link>
      </div>
    );
  }
    ...
    <div className="bg-white/5 border border-white/10 p-6 md:p-8 rounded-3xl flex flex-col h-[700px] max-h-[80vh]">
      <div className="flex justify-between items-center mb-6 pb-6 border-b border-white/10">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-xl md:text-2xl font-bold text-white">Ticket #{ticket.id}: {ticket.subject}</h2>
          </div>
      ...
      <div className="flex-1 overflow-y-auto pr-2 space-y-6 mb-6">
        {ticket.messages.map((msg: any) => (
          <div key={msg.id} className={`flex flex-col ${msg.isAdminMessage ? 'items-start' : 'items-end'}`}>
            <div className={`max-w-[80%] rounded-2xl p-4 ${msg.isAdminMessage ? 'bg-white/10 rounded-tl-none' : 'bg-violet-600 rounded-tr-none'}`}>
              <div className="flex justify-between items-center gap-4 mb-2">
                <span className={`text-xs font-bold ${msg.isAdminMessage ? 'text-violet-400' : 'text-white/80'}`}>
                  {msg.senderName}
                </span>
                <span className="text-[10px] text-white/50">{new Date(msg.createdAt).toLocaleTimeString()}</span>
              </div>
                ...
                <div className="mt-2">
                  {msg.attachmentType === 'image' ? (
                    <a href={msg.attachmentUrl} target="_blank" rel="noreferrer">
                      <img src={msg.attachmentUrl} alt="Attachment" className="max-w-full rounded-xl max-h-48 object-cover border border-white/10" />
                    </a>
                  ) : msg.attachmentType === 'audio' ? (
                    <audio src={msg.attachmentUrl} controls className="max-w-full h-10" />
                  ) : (
                    <a href={msg.attachmentUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-sm text-white/80 hover:text-white underline">
                      📎 View Attachment
                    </a>
                  )}
                </div>
      ...
      {ticket.status !== 'Closed' ? (
        <form onSubmit={handleSend} className="mt-auto bg-white/5 border border-white/10 p-3 rounded-2xl flex flex-col gap-3">
          {attachment && (
            <div className="flex items-center gap-2 px-3 py-2 bg-white/5 rounded-xl text-xs text-white/80">
              📎 {attachment.name}
              <button type="button" onClick={() => setAttachment(null)} className="ml-auto text-red-400 hover:text-red-300">✕</button>
            </div>
          )}
