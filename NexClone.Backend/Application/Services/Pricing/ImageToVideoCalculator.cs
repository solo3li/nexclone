...
namespace NexClone.Backend.Application.Services.Pricing
{
    public class ImageToVideoCalculator : IToolCostCalculator
    {
        ...
        {
            var setting = await _context.ImageToVideoSettings.FirstOrDefaultAsync();
            if (setting != null && !setting.IsActive)
                return new PricingResult { IsAllowed = false, ErrorMessage = "Image to Video is currently disabled." };
