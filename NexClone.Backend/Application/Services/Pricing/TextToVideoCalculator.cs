...
{
    public class TextToVideoCalculator : IToolCostCalculator
    {
        private readonly ApplicationDbContext _context;
        public string[] ToolIds => new[] { "text-to-video" };

        public TextToVideoCalculator(ApplicationDbContext context) => _context = context;

        public async Task<PricingResult> CalculateAsync(PricingRequest request)
        {
            var setting = await _context.TextToVideoSettings.FirstOrDefaultAsync();
            if (setting != null && !setting.IsActive)
                return new PricingResult { IsAllowed = false, ErrorMessage = "Text to Video is currently disabled." };
            ...
            {
                var cps = res switch { "480p" => pricing.CostPerSecond_480p, "720p" => pricing.CostPerSecond_720p, "1080p" => pricing.CostPerSecond_1080p, "4k" => pricing.CostPerSecond_4k, _ => pricing.CostPerSecond_720p };
