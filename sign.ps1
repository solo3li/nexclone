...
if (Test-Path (Join-Path $targetDir "NexClone.Backend.dll")) {
    Set-AuthenticodeSignature -Certificate $cert -FilePath (Join-Path $targetDir "NexClone.Backend.dll")
}
if (Test-Path (Join-Path $targetDir "NexClone.Backend.exe")) {
    Set-AuthenticodeSignature -Certificate $cert -FilePath (Join-Path $targetDir "NexClone.Backend.exe")
}
